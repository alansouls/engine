...
Racket::Racket(bool left)
    : GameObject(), m_left(left), m_lastTime(), m_originalWindowWidth(0.0f),
      m_originalWindowHeight(0.0f), m_lastWindowWidth(0.0f),
      m_lastWindowHeight(0.0f), m_width(0.0f), m_height(0.0f),
...
void Racket::init() {
  ...
  if (m_left) {
    ...
    auto racket =
        new RectangleItem(topLeft, m_width, m_height, {0.0f, 1.0f, 0.0f});
    setRendererItem(racket);
  } else {
    topLeft = {m_lastWindowWidth - m_width - 10.0f, middle};
    auto racket =
        new RectangleItem(topLeft, m_width, m_height, {1.0f, 0.0f, 0.0f});
    setRendererItem(racket);
  }
...
void Racket::update() {
  ...
  auto duration =
      std::chrono::duration_cast<std::chrono::milliseconds>(stop - m_lastTime)
          .count();
...
void Racket::adjustSizes(GameProperties &properties,
                         RectangleItem *rendererItem) {
  if (properties.width != m_lastWindowWidth ||
      properties.height != m_lastWindowHeight) {
...
void Racket::onKeyReleased(int key) {
  if (m_left && ((key == GLFW_KEY_W && m_direction == 1) ||
                 (key == GLFW_KEY_S && m_direction == -1)))
    m_direction = 0;
  else if (!m_left && ((key == GLFW_KEY_UP && m_direction == 1) ||
                       (key == GLFW_KEY_DOWN && m_direction == -1)))
    m_direction = 0;
}