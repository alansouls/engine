...
public:
  ...
  virtual void
  drawFrame(const std::vector<GraphicsOperation *> &updateOperations) = 0;
...
protected:
  ...
  static void framebufferResizeCallback(GLFWwindow *window, int width,
                                        int height) {
    auto driver =
        reinterpret_cast<GraphicsDriver *>(glfwGetWindowUserPointer(window));
