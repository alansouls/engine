...
VulkanDriver::VulkanDriver(const std::vector<const char *> requiredExtensions,
    ...
    : GraphicsDriver(window, options), m_validationLayers(validationLayers),
      m_deviceExtensions(deviceExtensions),
      m_requiredExtensions(requiredExtensions), m_instance(),
      m_debugMessenger(), m_surface(), m_physicalDevice(), m_logicalDevice(),
      m_graphicsQueue(), m_presentQueue(), m_swapChain(),
      m_swapChainImageFormat(VK_FORMAT_UNDEFINED), m_swapChainExtent({0, 0}),
      m_swapChainImages(), m_swapChainImageViews(), m_renderPass(),
      m_defaultPipelineLayout(), m_defaultGraphicsPipeline(),
      m_swapChainFramebuffers(), m_commandPool(), m_commandBuffers(),
      m_imageAvailableSemaphores(), m_renderFinishedSemaphores(),
      m_inFlightFences(), m_currentFrame(0), m_descriptorSetLayout(),
      m_circlePipelineLayout(), m_circleGraphicsPipeline(),
      m_extentFactorHeight(1.0f), m_extentFactorWidth(1.0f), m_primitives(),
      m_camera() {
...
void VulkanDriver::cleanup() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    for (auto &elements : m_elementsByType) {
      for (auto &element : elements.second) {
        ...
        vkFreeMemory(m_logicalDevice, element->storageBuffersMemory[i],
                     nullptr);
      }
    }
  }
  ...
  for (auto &elements : m_elementsByType) {
    for (auto &element : elements.second) {
      vkDestroyDescriptorPool(m_logicalDevice, element->descriptorPool,
                              nullptr);
    }
  }
...
void VulkanDriver::drawFrame(
    const std::vector<GraphicsOperation *> &updateOperations) {
  ...
  vkWaitForFences(m_logicalDevice, 1, &inFlightFence, VK_TRUE,
                  std::numeric_limits<uint64_t>::max());

  uint32_t imageIndex;

  VkResult result = vkAcquireNextImageKHR(
      m_logicalDevice, m_swapChain, std::numeric_limits<uint64_t>::max(),
...
void VulkanDriver::waitIdle() { vkDeviceWaitIdle(m_logicalDevice); }

void VulkanDriver::createInstance() {
  if (m_options.debugModeOn && !checkValidationLayerSupport()) {
    throw std::runtime_error(
        "Some of the specified validation layers are not supported!");
  }
...
VkExtent2D
VulkanDriver::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
...
void VulkanDriver::createDefaultGraphicsPipeline() {
  ...
  createGraphicsPipeline(m_options.defaultVertexShader,
                         m_options.defaultFragmentShader, bindingDescription,
                         attributeDescriptions.data(),
                         attributeDescriptions.size(), m_defaultPipelineLayout,
...
void VulkanDriver::createCircleGraphicsPipeline() {
  ...
  createGraphicsPipeline(m_options.circleVertexShader,
                         m_options.circleFragmentShader, bindingDescription,
                         attributeDescriptions.data(),
                         attributeDescriptions.size(), m_circlePipelineLayout,
                         m_circleGraphicsPipeline);
}

void VulkanDriver::createGraphicsPipeline(
    const std::vector<char> &vertexShaderBuffer,
  ...
  vertexInputInfo.vertexAttributeDescriptionCount =
      static_cast<uint32_t>(attributeDescriptionsSize);
...
void VulkanDriver::drawElements(VkCommandBuffer commandBuffer,
                                ElementType type) {
  ...
  VkPipelineLayout pipelineLayout =
      (type == ElementType::Quad ? m_defaultPipelineLayout
  ...
  for (auto element : elements) {
    ...
    vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(data.indicesSize),
                     element->instanceData.size(), 0, 0, 0);
  }
}
...
void VulkanDriver::updateVertexBuffer(GraphicElement *element) {
  ...
  case ElementType::Quad: {
    ...
    vertexData = new Vertex[4]{
        ...
        {glm::vec2(normalizedTopLeft.x + normalizedWidth,
                   normalizedTopLeft.y + normalizedHeight)},
        {glm::vec2(normalizedTopLeft.x,
                   normalizedTopLeft.y + normalizedHeight)}};
  } break;
  case ElementType::Circle: {
  ...
  vkMapMemory(m_logicalDevice, stagingBufferMemory, 0, bufferSize, 0,
              &vertexBufferData);
  ...
  createBuffer(bufferSize,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, data.vertexBuffer,
...
void VulkanDriver::updateIndexBuffer(GraphicElement *element) {
  ...
  createBuffer(bufferSize,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, data.indexBuffer,
...
void VulkanDriver::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                              VkDeviceSize size) {
  VkCommandBufferAllocateInfo allocInfo{};
  allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
  allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void VulkanDriver::createUniformBuffers() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    vkMapMemory(m_logicalDevice, m_camera.uniformBuffersMemory[i], 0,
                bufferSize, 0, &m_camera.uniformBuffersMapped[i]);
  }
}
...
void VulkanDriver::createStorageBuffers(GraphicElement *element) {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    vkMapMemory(m_logicalDevice, element->storageBuffersMemory[i], 0,
                bufferSize, 0, &element->storageBuffersMapped[i]);
  }
}
...
void VulkanDriver::createDescriptorPool(GraphicElement *element) {
  ...
  if (vkCreateDescriptorPool(m_logicalDevice, &poolInfo, nullptr,
                             &element->descriptorPool) != VK_SUCCESS) {
...
void VulkanDriver::createDescriptorSets(GraphicElement *element) {
  ...
  if (vkAllocateDescriptorSets(m_logicalDevice, &allocInfo,
                               element->descriptorSets.data()) != VK_SUCCESS) {
...
void VulkanDriver::performOperation(GraphicsOperation *operation) {
  ...
  case GraphicsOperation::Type::Add: {
    if (m_elementsByType.contains(operation->elementType.value())) {
      ...
      m_elementsByType[operation->elementType.value()] =
          std::vector<GraphicElement *>();
    ...
    glm::mat4 model =
        glm::translate(glm::mat4(1.0f), operation->transformPosition.value());
    model = glm::scale(model, operation->transformScale.value());
    element->instanceData.push_back(
        {.model = model, .inColor = operation->color.value()});
    ...
    operation->result =
        element->instanceData.size() +
        ((size_t)operation->elementType.value() * MAX_INSTANCES);
  } break;
...
void VulkanDriver::updateStorageBuffer(GraphicElement *element,
  ...
  memcpy(element->storageBuffersMapped[currentImage], instanceDataArray,
