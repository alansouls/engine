...
typedef char char_t;
    ...
    //      Only command-line arguments which are accepted by runtime installation are supported, SDK/CLI commands are
    //      not supported. For example 'app.dll app_argument_1 app_argument_2`.
    //    parameters
    ...
    //    Success_DifferentRuntimeProperties - Config has runtime properties that differ from already initialized
    //    hosting components CoreHostIncompatibleConfig         - Config is incompatible with already initialized
    //    hosting components
    //
    ...
    typedef void(HOSTFXR_CALLTYPE *hostfxr_resolve_frameworks_result_fn)(
        const struct hostfxr_resolve_frameworks_result *result, void *result_context);
